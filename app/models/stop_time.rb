class StopTime < ApplicationRecord
  ...
  def self.next(from, direction, target_datetime)
    if from.is_a? Stop
    ...
    StopTime.joins(:trip).where(trips: { direction_id: direction, service_id: Calendar.active_today.service_id }).where(stop_id: from_id).older_than(target_datetime.seconds_since_midnight).order(:seconds_since_midnight)
  end
